...
show_help() {
    ...
    echo ""
    echo "Tunnels SSH:"
    echo "  Les tunnels SSH permettent le port forwarding via"
    echo "  un proxy de rebond (ex: accès BDD distante)."
    echo "  Ils peuvent dépendre d'un VPN (connexion automatique)."
    echo ""
    echo "Dépendances:"
    echo "  Un VPN ou tunnel peut dépendre d'une autre connexion."
    echo "  La dépendance est vérifiée automatiquement à la connexion"
    echo "  et à la déconnexion."
    echo ""
    echo "Configuration:"
    echo "  $VPN_CONF"
    echo ""
